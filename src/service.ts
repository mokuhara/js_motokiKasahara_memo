...
const editMemoLister = () => {
  ...
  editBtns.forEach((editBtn) => {
    editBtn.addEventListener("click", (event) => {
      ...
      if (!_memo) throw new Error("failed get memo");
      _memo.edit();
...
//ここから内部で使用する関数
const _createMemo = (memo: memoData) => {
  const _memo = new Memo(memo);
  _memo.add();
  _deleteInputValue();
};

const _updateMemo = (memo: memoData) => {
  const _memo = new Memo(memo);
  _memo.update();
  _deleteInputValue();
...
const _getTargetMemo = (element: Event) => {
  const _element = element.currentTarget as HTMLElement;
  if (!element || !_element.dataset || !_element.dataset.memo) return;
...
const _isNullValidation = (target: HTMLInputElement, message: string) => {
